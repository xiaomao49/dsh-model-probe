      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          staleCount > 0
            ...
            : null,
          // 配了探测专属头的 provider 说一声：那些头只用于本插件的探测请求，
          // 不影响 DSH 的真实调用。值本身不显示（可能含凭据）。
          (status.policy?.probeHeaderProviders ?? []).length > 0
            ? React.createElement(
                'span',
                null,
                React.createElement('br'),
                `以下 provider 另有探测专属请求头（只用于本插件的探测，不影响 DSH 正常调用）：${(status.policy?.probeHeaderProviders ?? []).join('、')}。`,
              )
            : null,
          React.createElement('br'),
