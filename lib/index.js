...
export function apply(ctx, config = {}) {
  ...
  const { enabled, stale } = enabledIntersection(startupSection.section, startupSection.raw === true)
  console.log(
  ...
  } else if (startupSection.raw !== true) {
    // 加载时 llm-pi-ai 命名空间常常还没注册，这时读不到 provider 列表。说清楚是
    // "这次没读到"，而不是留下一个"一切正常"或"全都没了"的错误印象。
    console.log(
      `[dsh-model-probe] 加载时读不到 llm-pi-ai 的用户层（${startupSection.reason ?? '尚未注册'}），` +
        `暂不判断探测白名单；设置页每次打开都会重新读取。`,
    )
  }
}
