...
function buildStatus(ctx) {
  ...
  const providers = listProviders(section).map((id) => {
    ...
    return {
      ...
      probeEnabled: state.policy.enabledProviders.includes(id),
...
export function apply(ctx, config = {}) {
  ...
  const enabled = state.policy.enabledProviders
  console.log(
    ...
    defineTool({
      ...
      execute: async (args) => {
        ...
        return {
          ...
          hint:
            result.opCount > 0
              ? `发现 ${result.opCount} 处待修正。调用 model_probe_apply 并传 confirm=true 才会写入配置。`
              : '未发现需要修正的字段。',
        }
      },
    }),
  ...
  route(
    ...
    guard(async (req, res) => {
      ...
      if (Array.isArray(body.enabledProviders)) {
        patch.enabledProviders = body.enabledProviders.filter((x) => typeof x === 'string')
      }
