...
import { scanProvider, planOps, describeOps, compact, DEFAULT_POLICY } from './scan.js'
...
export const PolicySchema = z.object({
  ...
  /**
   * 探测请求专属的额外头：{ providerId: { header: value } }。
   *
   * 刻意放在本插件自己的命名空间里，而不是 llm-pi-ai 的 provider.headers：
   * 后者的值 DSH 的真实模型调用也会带上，会把"按会话动态发头"的机制顶掉
   * （静态值优先），让所有会话塌缩成一个缓存亲和桶。写在这里只影响探测。
   *
   * 这些值可能含凭据，因此【绝不】经 HTTP 状态接口返回，也绝不进日志。
   *
   * 这里刻意用 z.any() 而不是严格的嵌套字典：settings.yaml 里一个手写的头写错
   * 形状（`probeHeaders: {p1: "oops"}`）会让 schema 解析抛错，进而让整个
   * model-probe 命名空间安装失败——后果不只是这个头被忽略，而是 enabledProviders
   * 一起失效、开关再也存不下来，而报错信息只会说 schemastery 内部的话。畸形内容
   * 交给 normalizeProbeHeaders 逐条丢弃，坏一个头不该掀翻整张表。
   */
  probeHeaders: z.any().default({}),
})
...
const state = {
  ...
  persist: false,
  /**
   * 每个 provider 最近一次成功扫描的结果，连同取证时的 llm-pi-ai 修订号。
   *
   * 写入必须写「用户刚刚审阅过的那一次扫描」，而不是重新扫一遍：
   *   - 重扫要再花一次图像实证的输出 token；
   *   - 行为型探针可能给出与上次不同的结论（实测遇到过图像判定翻转），于是
   *     「界面上看到的」和「写进去的」会不一致——这种不一致最伤信任。
   * 但也不能无条件信任缓存：配置在扫描之后被改过，证据就过期了。所以绑定修订号，
   * 修订号对不上就重新取证。
   */
  lastScans: new Map(),
}
...
function send(res, status, body) {
  ...
  // 收口点：客户端边界的判据与工具通道相同（@deepseek-ai/dsh-api-gateway 用的是
  // 同一个 isJsonValue），所以 HTTP 响应也必须无损，不能只在工具出口收。
  res.end(JSON.stringify(compact(body)))
}
 ...
 *   白名单里已不存在于配置中的名字。
 */
function enabledIntersection(section, readable = true) {
  const stored = Array.isArray(state.policy.enabledProviders) ? state.policy.enabledProviders : []
  // 读不到 provider 列表时不下任何判断。这条是实测摔出来的：插件在 apply() 里读
  // 设置时，llm-pi-ai 命名空间往往还没注册，readPiSection 会回退成 { providers: {} }，
  // 于是"交集为空"被当成"白名单里所有 provider 都已被删除"，启动日志赫然写着
  // 「探测白名单里有 2 个 provider 已不在 llm-pi-ai 配置中」——而它们配置得好好的。
  // 读不到的诚实说法是"不知道"，不是"它们没了"。
  if (readable !== true) return { enabled: [...stored], stale: [], readable: false }
...
function buildStatus(ctx) {
  ...
  // raw 为真才说明读到了用户层那份权威配置；否则宁可不下判断。
  const { enabled, stale } = enabledIntersection(section, raw === true)
  ...
  // probeHeaders 的值可能含凭据，绝不能经状态接口返回。这里把它从策略副本里
  // 摘掉，只留"哪些 provider 配了探测头"这个事实供界面展示——形状上与既有的
  // 「凭据不出现在任何返回值里」一致。
  const { probeHeaders, ...policyForClient } = state.policy
  return {
    ok: true,
    providers,
    policy: {
      ...
      probeHeaderProviders: Object.keys(probeHeaders ?? {}),
    },
...
async function runScan(ctx, providerId, options = {}) {
  ...
  try {
    // 修订号必须在扫描**之前**取。扫描要发好几个请求、耗时以秒计；若期间配置被
    // 改动，取自结束时刻的修订号会给"在旧配置下取得的证据"贴上"对新配置有效"的
    // 标签，而写入正是靠这个标签决定要不要复用缓存。取起始值则只会保守失效——
    // 宁可多扫一次，不可写错一次。
    const revisionAtStart = readRevision(ctx)
    ...
    // 记下这次取证，供随后的写入复用（见 state.lastScans 的说明）。
    state.lastScans.set(providerId, { revision: revisionAtStart, scan })
...
async function runApply(ctx, providerId, options = {}) {
  ...
  // 注意：凭据只在确实要发请求的分支里解析（见下）。复用已审阅的扫描时不发任何
  // 请求，也就不该去动凭据——凭据按次解析是硬纪律，不该有"解析了但没用上"的路径。

  //
  // 曾经这里是无条件重扫、并且刻意关掉视觉探针，注释还写着「结论已在上一次扫描
  // 得到」——但代码并没有把上次的结论带过来，于是界面里明明列着「实测支持图像」，
  // 点确认写入却报「已修正 0 处」：input 只在视觉探针里取证，关掉它，这个字段就
  // 永远写不进去。实测撞到过这个静默失效。
  const cached = state.lastScans.get(providerId)
  const revision = readRevision(ctx)
  const reviewed =
    cached !== undefined && revision !== undefined && cached.revision === revision

  if (!reviewed && state.scanning) {
    // 写入需要重新取证时，和扫描共用同一把闸：否则"扫描还在跑、顺手点写入"
    // 会并发发出两轮真实计费请求。复用缓存则不涉及发请求，无需等待。
    return { ok: false, error: '已有扫描在进行中，请等它结束再写入' }
  }
  ...
  const scan = reviewed
    ...
    : await scanProvider({
        provider: providerId,
        section,
        apiKey: await resolveApiKey(ctx, providerId, profile.apiKeyEnv),
        // 没有可复用的结论时必须完整取证，否则又会丢掉 input。
        policy: { ...state.policy },
        onProgress: () => {},
      })
  if (scan.ok !== true) return scan
  ...
  return {
    ...
    changes,
    // 如实说明这些结论来自哪一次取证：界面上刚看过的那次，还是写入前重扫的。
    evidence: reviewed ? 'reviewed-scan' : 'fresh-scan',
  }
}
...
function normalizePolicy(config = {}) {
  ...
  return {
    ...
    probeHeaders: normalizeProbeHeaders(config.probeHeaders),
  }
}

/**
 * 把 probeHeaders 归一化成 { provider: { header: value } }。
 *
 * 手写配置什么都可能塞进来，而这里的东西会被直接送进 HTTP 头，所以只接受
 * 两层纯对象 + 非空字符串：其余一律丢弃而不是抛错——一个写错的头不该让整个
 * 插件加载失败，静默忽略它并继续服务其余 provider 才是合理的降级。
 */
function normalizeProbeHeaders(raw) {
  if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return {}
  const out = {}
  for (const [provider, headers] of Object.entries(raw)) {
    ...
    if (Object.keys(kept).length > 0) out[provider] = kept
  }
  return out
}
...
export function apply(ctx, config = {}) {
  ...
  // 无论来源是组合配置还是 settings，都过同一套归一化。settings 那条原本直接返回
  // schema 解析结果，于是 probeHeaders 的"畸形就丢弃"只在组合配置这条路上生效，
  // 而用户实际改的多半是 settings.yaml。
  const readPolicy = () => normalizePolicy(currentPolicy() ?? {})
  state.persist = false
  ...
  // 启动这一条要一次说清，别让两行互相打架：加载时 llm-pi-ai 命名空间常常还没注册，
  // 这时读到的只是白名单原文，没有任何存在性核对。分开写会变成上一行说"探测当前
  // 开启：A、B"、下一行又说"读不到、暂不判断"——两句话都真，合起来却自相矛盾。
  const startupSection = readPiSection(ctx)
  const readable = startupSection.raw === true
  const { enabled, stale } = enabledIntersection(startupSection.section, readable)
  const enabledText = enabled.length > 0 ? enabled.join(', ') : '（无，需在设置页逐 provider 开启）'
  console.log(
    `[dsh-model-probe] 已加载；工具 model_probe_status/scan/apply 已注册。` +
      (readable
        ? `探测当前开启：${enabledText}`
        : `探测白名单：${enabledText}（加载时 llm-pi-ai 尚未注册，未做存在性核对；设置页每次打开都会重新读取）`) +
      `；策略持久化：${state.persist === true ? '已启用（写入 settings.yaml 的 model-probe 段）' : '不可用（开关仅本进程有效）'}`,
  )
  // 白名单里留着已删除的 provider 时明确说出来：否则用户只会在日志里看到探测
  // 开着，而设置页又不显示那一条，无从下手。只有确实读到配置时才可能判断。
  if (readable && stale.length > 0) {
    console.log(
...
function registerTools(ctx) {
  ...
  ctx.tools.register(
    defineTool({
      ...
      parameters: {},
      output: statusSchema,
      execute: async () => compact(buildStatus(ctx)),
    }),
  )
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        '把实测结论写入 settings.yaml，修正越界或不符的模型配置（如某个模型的 maxTokens 超过端点合法上限导致请求全部失败）。写入用的证据优先是刚跑过的那次 model_probe_scan（配置未变时直接复用，不重复发请求）；没有可复用的结论时会先完整取证（含图像实证）。写入前自动备份 settings.yaml，并带乐观锁——若配置在此期间被改动则放弃写入。必须显式传 confirm=true。',
      parameters: {
      ...
      execute: async (args) => {
        ...
        // 收口点：`changes` 里的 `from` 在"字段是新增的"时不存在，`verdict` 在
        // 该字段没有对应 finding 时也不存在——两者都会变成 undefined 属性。
        return compact(await runApply(ctx, args.provider, { models: args.models, only: args.only }))
      },
    }),
  )
}
...
function registerRoutes(ctx) {
  ...
  route(
    ...
    guard(async (req, res) => {
      ...
      // 探测头走同一套归一化：只收两层纯对象 + 非空字符串，其余丢弃。写进来
      // 的值同样不会出现在任何响应里（见 buildStatus）。
      if (body.probeHeaders !== undefined) {
        patch.probeHeaders = normalizeProbeHeaders(body.probeHeaders)
      }
