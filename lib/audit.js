...
export function summarize(findings) {
  ...
  // 已验证字段数 = 拿到过确凿证据的字段（unknown 之外的一切）。
  //
  // 这个数字必须单独暴露出来。否则"没有任何差异"与"什么都没测出来"在结果里长得
  // 一模一样 —— 前者是"配置正确"，后者是"探测失败了"。把后者显示成绿色的
  // "无需改动"正是本插件最不该犯的错误，而它实际发生过：端点全部 fetch failed，
  // 界面却报"配置与实测一致"。
  const verified = findings.filter((f) => f.verdict !== VERDICT.UNKNOWN).length

  return {
    counts,
    verified,
    unknown: counts[VERDICT.UNKNOWN] ?? 0,
