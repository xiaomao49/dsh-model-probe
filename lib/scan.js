...
/**
 * 递归清洗成可无损 JSON 化的结构，供跨边界返回前收口。
 *
 * 为什么必须有这一步：DSH 的取值边界（`dsh-util-values` 的 `isJsonValue` /
 * `snapshotJsonValue`）把两类值判为**非法**——`undefined`（既不是 null，也不是
 * boolean/string/number/object）与非有限数字（`NaN`/`±Infinity`；`-0` 同样不合格，
 * 因为它与 `0` 无法在 JSON 往返中区分）。任何携带它们的对象都过不了闸门，工具通道
 * 会直接抛 `returned invalid output: value is not lossless JSON`。
 *
 * 而 `JSON.stringify` 的默认行为与之不一致：它静默丢掉 `undefined` 属性，把
 * `NaN`/`Infinity` 写成 `null`，把 `-0` 写成 `0`。本插件原先正是照这套直觉写的
 * （可选字段直接写进对象），撞上的是"不丢才合格"的判据。
 *
 * 症状因此是反直觉的：探针全失败时所有 finding 的 verdict 都是 `unknown`，被出口
 * 过滤器滤光、对象里一个非法值都没有，工具反而能用；一旦真的取证到东西，
 * `limit`/`from`/`note` 之类的可选字段带着 `undefined` 出现，工具就挂了。
 *
 * 统一在最外层收口，而不是逐字段条件展开（`...(x === undefined ? {} : { x })`）：
 * 后者每新增一个可选字段就要记得写一次，漏掉一次就是一次线上不可用。
 *
 * 清洗规则（与其判据一一对应）：
 *   - `undefined` → 丢掉该键；
 *   - `NaN` / `±Infinity` → 丢掉该键（写成 null 是**伪造数据**，宁可让界面显示"—"）；
 *   - `-0` → 归一为 `0`（JSON 里 `-0` 与 `0` 无法区分，丢弃符号不是丢信息）；
 *   - `null` → 保留。它是合法 JSON 值，本插件用 `?? null` 表示"明确无值"，
 *     把它一并丢掉会让界面分不清"没有值"与"没这个字段"。
 */
export function compact(value) {
  ...
  if (Array.isArray(value)) {
    // 数组项被清洗成 undefined 时**移除**而不是留下空洞：`[1, NaN, 2]` 若写成
    // `[1, undefined, 2]`，JSON 化后是 `[1, null, 2]`——那是伪造数据，而且
    // 网关对数组另有"稠密"要求。本插件的数组都是清单（models / findings / plan），
    // 下标的语义由元素自身的 index 字段承载，移除元素不影响含义。
    const out = []
  ...
  for (const [key, item] of Object.entries(value)) {
    const cleaned = compact(item)
    if (cleaned === undefined) continue
    // `__proto__` 与 `constructor` 必须用 defineProperty 写：普通赋值 `out[key] =`
    // 对 `__proto__` 会走 Object.prototype 的 setter，把目标对象的**原型**改掉
    // （JSON.parse 产出的 `__proto__` 是自有数据属性，恰好会走到这条路径）。
    // 这里要的是"原样保留一个自有键"，不是改原型。
    if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
      Object.defineProperty(out, key, { value: cleaned, enumerable: true, configurable: true, writable: true })
      continue
    }
    out[key] = cleaned
  }
...
export const DEFAULT_POLICY = {
  ...
  /**
   * 探测请求专属的额外头，按 provider 分组：{ providerId: { header: value } }。
   *
   * 为什么需要它：有些网关除 API key 外还强制要求别的头（实测 opencode.ai 的
   * Console Go 缺 x-opencode-session 就一律 400），而给探测请求补这个头只有两条
   * 路——写进 llm-pi-ai 的 provider headers，或者写在这里。前者是错的：那份配置
   * DSH 的真实模型调用也会带上，于是"按会话动态发头"的机制（配置值优先、见
   * dsh-opencode-session）会被静态值顶掉，所有会话塌缩成一个缓存亲和桶。
   * 写在这里则只影响本插件的探测请求，真实运行完全不受影响。
   */
  probeHeaders: {},
...
export async function scanProvider({ provider, section, apiKey, policy, signal, onProgress }) {
  ...
  //
  // 在此之上再叠一层本插件专属的 probeHeaders：它只用于探测，不写进 llm-pi-ai，
  // 因此不会反过来影响 DSH 的真实调用。同名时探测专属值胜出——那是用户为探测
  // 显式指定的值。
  const probeHeaders = {
    ...(profile.headers ?? {}),
    ...(policy.probeHeaders?.[provider] ?? {}),
  }
